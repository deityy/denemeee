...
mongoose.connect('mongodb+srv://orchaisbot:fantasieloser@cluster0.sc3bn.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});
const Database = require("./models/role.js");

client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Orchais ❤️ Guevara" }, status: "idle" });
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#00ffdd").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`Orchais ❤️ Guevara`).setTimestamp();
...
// Rolü oluşturtup geri dağıtma kısmı
  if(command === "kur" || command === "kurulum" || command === "backup" || command === "setup") {
    ...
    Database.findOne({guildID: ayarlar.guildID, roleID: args[0]}, async (err, roleData) => {
      ...
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(); } else { message.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(err => {}); };
    });
  };
});
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "CEZALANDIRILDI" }).catch();
};
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  if (newMember.roles.cache.size > oldMember.roles.cache.size) {
    ...
    if (yetkiPermleri.some(p => !oldMember.hasPermission(p) && newMember.hasPermission(p))) {
      ...
      let logKanali = client.channels.cache.get(ayarlar.logChannelID);
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Yetki Verildi!').setDescription(`${newMember} (${newMember.id}) üyesine ${entry.executor} (${entry.executor.id}) tarafından sağ tık yetki verildi! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(); } else { newMember.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Yetki Verildi!').setDescription(`${newMember} (${newMember.id}) üyesine ${entry.executor} (${entry.executor.id}) tarafından sağ tık yetki verildi! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(err => {}); };
    };
  };
});
// Rol açtırmama
client.on("roleCreate", async role => {
  ...
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Oluşturuldu!').setDescription(`${entry.executor} (${entry.executor.id}) tarafından bir rol oluşturuldu! Oluşturan kişi jaile atıldı ve rol silindi.`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(); } else { role.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Oluşturuldu!').setDescription(`${entry.executor} (${entry.executor.id}) tarafından bir rol oluşturuldu! Oluşturan kişi jaile atıldı ve rol silindi.`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(err => {}); };
});
...
function ytKapat(guildID) {
  ...
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('İzinler Kapatıldı!').setDescription(`Rollerin yetkileri kapatıldı!`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(); } else { channel.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('İzinler Kapatıldı!').setDescription(`Rollerin yetkileri kapatıldı!`).setFooter(`Orchais ❤️ Guevara`).setTimestamp()).catch(err => {}); };
};
